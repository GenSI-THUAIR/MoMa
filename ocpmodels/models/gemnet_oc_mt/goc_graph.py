...
def _generate_graph(
    ...
    *,
    ...
    # data.tags = torch.Tensor(data.tags)
    if pbc:
        ...
        if type(data.pos) == np.ndarray:
            data.pos = torch.from_numpy(data.pos)
        elif type(data.pos) == Tensor:
            data.pos = data.pos
        else:
            print(f"Unrecognized data type {type(data.pos)}")
            exit(0)
        data.tags = torch.Tensor(data.tags)
        data.atomic_numbers = torch.Tensor(data.atomic_numbers)
