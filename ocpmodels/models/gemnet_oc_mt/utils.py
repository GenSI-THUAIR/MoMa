...
def get_max_neighbors_mask_tensor(
    natoms: torch.Tensor,
    index: torch.Tensor,
    ...
    num_atoms = int(natoms.sum())

    # Get number of neighbors
    # segment_coo assumes sorted index
    ones = index.new_ones(1).expand_as(index)
    num_neighbors = segment_coo(ones, index, dim_size=num_atoms)
    max_num_neighbors = int(num_neighbors.max())
    ...
    if (max_num_neighbors_threshold >= max_num_neighbors).all():
        mask_num_neighbors = torch.ones_like(num_neighbors, dtype=torch.bool)
...
def get_inner_idx(idx, dim_size):
    ...
    # ones_cpu = ones.cpu()
    # idx_cpu = idx.cpu()
    # nn_cpu = segment_coo(ones.cpu(), idx.cpu(), dim_size=dim_size)
    # print(ones_cpu, idx_cpu, nn_cpu, nn_cpu.shape, nn_cpu.sum())
