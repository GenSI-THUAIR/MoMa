...
@registry.register_model("gemnet_oc_adapter")
class GemNetOCBackbone(BaseModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        print(f"Initializing GemNetOCBackbone with adapter.")
        super().__init__(
        ...
        AdapterLayers = []
        for _ in range(num_blocks + 1):
            AdapterLayers.append(AdapterLayer(d_model=emb_size_atom,
                dropout=0.1,
                bottleneck=emb_size_atom//2,
                AdapterLayernorm_option="in"))
        self.AdapterLayers = nn.ModuleList(AdapterLayers)
    ...
    def _forward(self, data: Batch):
        ...
        for i in range(self.num_blocks):
            ...
            # (nAtoms, emb_size_atom), (nEdges, emb_size_edge)
            x_E = self.AdapterLayers[i+1](x_E)
