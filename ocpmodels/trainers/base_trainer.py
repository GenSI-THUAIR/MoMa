...
"""
Base trainer class for Open Catalyst Project models.

This module provides the base trainer class that handles common training functionality
like data loading, model training, validation, and checkpointing.
"""
...
class BaseTrainer(ABC):
    ...
    This class provides the foundation for all trainer implementations in the OCP project.
    It handles common tasks like data loading, model training, validation, and checkpointing.
    ...
    Attributes:
        ...
        output_targets (Dict[str, Any]): Dictionary mapping output names to their targets
        normalizers (Dict[str, Any]): Dictionary of normalizers for different outputs
        ema (Optional[ExponentialMovingAverage]): Exponential moving average for model weights
    ...
    def load(self) -> None:
        ...
        print("check:", self.config["task"]["base_checkpoint"]["src"])
        ...
        self.load_loss()
        # print("=" * 20 + "Ftart loading loss", "=" * 20)
        # print("=" * 20 + "Start loading optimizer", "=" * 20)
        self.load_optimizer()
        # print("=" * 20 + "Finish loading optimizer", "=" * 20)
        self.load_extras()
    ...
    def load_task(self):
        ...
        ## Assert that all targets, loss fn, metrics defined and consistent
    ...
    def load_model(self) -> None:
        ...
        # print("using gpu_nums:", self.config["gpu_num"])
        self.model = OCPDataParallel(
            self.model,
            output_device=self.device,
            num_gpus=1 if not self.cpu else 0,
            # num_gpus=self.config["gpu_num"] if not self.cpu else 0,
        )
        ...
        for epoch_int in range(
            ...
            for i in range(skip_steps, len(self.train_loader)):
                ...
                if (
                    ...
                    self.save(
                        checkpoint_file=f"checkpoint_{self.step}.pt", training_state=True  
                    )
    ...
    def validate(self, split: str = "val", disable_tqdm: bool = False):
        ...
        for i, batch in tqdm(
            ...
            label = batch[0][target_name].unsqueeze(1)
            labels.append(label)
        ...
        # pearson_correlation, _ = pearsonr(labels_np, preds_np)

        # print("Pearson correlation coefficient:", pearson_correlation)

        # result_path = self.config["task"]["result_path"]
    ...
    def predict(
        ...
        for i, batch_list in tqdm(
            ...
            batch_names = [batch_list[j].name for j in range(len(batch_list))]
            print(f"cif_names for batch {i}: {batch_names}.")

            ######  IGNORE sids for MOMA ######
            ...
            for target_key in self.config["outputs"]:
                ...
                if target_key in out:
                    ...
                    if self.normalizers.get(target_key, False):
                        print("Doing denormalization during prediction")
        ...
        for key in predictions:
            print(f"prediction for {key} is {predictions[key]}.")
