...
@registry.register_trainer("mt_matbench")
class MTTrainer(BaseTrainer):
    ...
    def load_checkpoint(self, checkpoint_path: str) -> None:
        ...
        _ = load_state_dict(self.model, new_dict, strict=False)
    ...
    def load_task(self):
        ...
        self.output_targets = {}
        # do not support different outputs in different tasks, need modify
    ...
    def _compute_loss(
        ...
        one_hot_node = one_hot[batch_idx]  # (n_atoms, t) only for atom-based target e.g. forces
        ...
        for loss_info in self.loss_fns:
            ...
            if reduction == "structure_wise_mean":
                ...
                y_loss_mask_natoms = scatter(
                    mask.float(), batch_idx, dim=0, reduce="sum"
                )  # (b, t)
                loss = _safe_divide(loss, y_loss_mask_natoms)  # (b, t)
                mask = y_loss_mask_natoms > 0.0  # (b, t)
    ...
    def _compute_per_task_metrics(
        ...
        per_task_y_maes = (
            self.val_per_task_y_maes
            if validate
            else self.train_per_task_y_maes
        )
        ...
        y_target = torch.cat(
            [batch.y.to(self.device) for batch in batch_list]
        )  # (n_atoms, 3)

        # print("*"*20 + "output y_target" + "*"*20)
        # print(y_target)

        y_target = einsum(
            y_target, one_hot, "b, b t -> b t"
        )  # (n_atoms, t, 3)

        # eval_on_free_atoms = self.typed_output_targets["y"].eval_on_free_atoms  # type: ignore
        # free_mask = None
        ...
        for task in self.multi_task_config.tasks:
            ...
            y_mae = per_task_y_maes[task.idx](
                outputs["y"][mask], y_target[mask]
            )
            if not validate:
                metrics = self.evaluator.update(
                    f"{task.name}_y_mae", y_mae.item(), metrics
                )
    ...
    def validate(self, split: str = "val", disable_tqdm: bool = False):
        ...
        for task in self.multi_task_config.tasks:
            ...
            y_mae = self.val_per_task_y_maes[task.idx]
            metrics = self.evaluator.update(
                f"{task.name}_y_mae",
                y_mae.compute().item(),
                metrics,
    ...
    def _compute_metrics(
        ...
        if (
            ...
            for task_idx in range(self.num_tasks):
                ...
                self.train_per_task_y_maes[task_idx].reset()
    ...
    def _forward(self, batch_list):
        outputs, _ = super()._forward(batch_list)
