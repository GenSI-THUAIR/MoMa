...
def load_scales_compat(
    ...
    for name, scale in scale_dict.items():
        ...
        loaded_factors.add(name)

    not_loaded_factors = set(scale_factors.keys()) - loaded_factors
    # logging.critical(
    #     f"Loaded the following scale factors: {list(loaded_factors)}.\n"
    #     f"Did not load the following scale factors: {list(not_loaded_factors)}."
    # )
