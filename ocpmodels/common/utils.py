...
def dict_set_recursively(dictionary, key_sequence, val) -> None:
    ...
    # Check if the current key includes list access, e.g., "val[0]"
    if "[" in top_key and "]" in top_key:
        list_key, index = top_key.split("[")
        index = int(index[:-1])  # Remove the closing "]" and convert to int

        # Ensure the list exists and has enough capacity
        if list_key not in dictionary or not isinstance(
            dictionary[list_key], list
        ):
            dictionary[list_key] = []
        while len(dictionary[list_key]) <= index:
            dictionary[list_key].append({})

        # If we're at the final key, set the value directly
        if len(key_sequence) == 0:
            dictionary[list_key][index] = val
        else:
            # Otherwise, recurse into the list element
            dict_set_recursively(
                dictionary[list_key][index], key_sequence, val
            )
    else:
        if len(key_sequence) == 0:
            dictionary[top_key] = val
        else:
            if top_key not in dictionary or not isinstance(
...
def setup_logging() -> None:
    ...
    if not root.hasHandlers():
        ...
        log_formatter = logging.Formatter(
            "| %(relativepath)s | %(lineno)s | (%(levelname)s): %(message)s",
            datefmt="%Y-%m-%d %H:%M:%S",
        )
